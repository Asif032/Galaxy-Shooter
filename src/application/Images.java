package application;

import javafx.scene.image.Image;

public class Images {
  static final Image PLAYER_IMG[] = {
      new Image("/images/ship1.png"),
      new Image("/images/ship2.png"),
      new Image("/images/ship3.png"),
      new Image("/images/ship4.png"),
  };
  
  static final Image ENEMY_IMG[] = {
    new Image("/images/enemy1.png"),
    new Image("/images/enemy2.png"),
    new Image("/images/enemy3.png"),
    new Image("/images/enemy4.png"),
    new Image("/images/enemy5.png")
  };
  
  static final Image BACK_IMG[] = {
      new Image("/images/bg1.jpg"),
      new Image("/images/bg5.jpg"),
      new Image("/images/bg3.jpg"),
      new Image("/images/bg2.jpg")
    };
  
  static final Image BULLET_IMG[] = {
      new Image("/images/Fire3Single.png"),
      new Image("/images/Fire2Single.png"),
      new Image("/images/Rocket1Single.png")
    };
  
  static final Image ICON_IMG = new Image("/images/icon.png");
  
  static final Image EXPLOSION_IMG = new Image("/images/explosion.png");

  
}

